...
sshre() { output=$(ssh "$@" 2>&1); if echo "$output" | grep -q "no matching host key type found"; then algorithms=$(echo "$output" | grep -oP 'Their offer: \K.*?(?=\r|\n)' | sed 's/ /,/g'); printf "Host %s\n HostKeyAlgorithms +%s\n" "$1" "$algorithms" | tee -a "$HOME/.ssh/config" >/dev/null; echo "HostKey config saved, reconnecting..."; ssh "$@"; elif echo "$output" | grep -q "no matching key exchange method found"; then algorithms=$(echo "$output" | grep -oP 'Their offer: \K.*?(?=\r|\n)' | sed 's/ /,/g'); printf "Host %s\n    KexAlgorithms +%s\n" "$1" "$algorithms" | tee -a "$HOME/.ssh/config" >/dev/null; echo "Kex config saved, reconnecting..."; ssh "$@"; else echo "$output"; fi; }
